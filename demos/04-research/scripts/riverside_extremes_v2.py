...
Sort by sentiment_net (raw score) to break ties in normalized score."""
...
# Sort by net score (ascending), break ties by normalized
riverside.sort(key=lambda r: (r["sentiment_net"], r["sentiment_normalized"]))

most_neg = riverside[:10]
most_pos = riverside[-10:][::-1]  # reverse so most positive first
...
def print_group(label, group):
    ...
    for i, r in enumerate(group, 1):
        print(f"\n--- #{i}: {r['name']} ({r['employee_id']}) | net={r['sentiment_net']} | norm={r['sentiment_normalized']:.3f} | cat={r['sentiment_category']} ---")
...
print_group("10 MOST NEGATIVE RIVERSIDE RESPONSES (by net score)", most_neg)
print_group("10 MOST POSITIVE RIVERSIDE RESPONSES (by net score)", most_pos)
...
def summarize(label, group):
    ...
    wings = Counter(str(r["building_wing"]) for r in group)
    ...
    print(f"  Origin sys quality: {dict(origin_q) if origin_q else 'none (no transfers)'}")
...
# ── Theme extraction ──
print(f"\n{'='*90}")
print("  THEMATIC COMPARISON")
print(f"{'='*90}")

theme_keywords = {
    "disruption to routine":       ["disrupt", "routine", "thrown off", "adjustment", "adjusting", "threw off", "restructur"],
    "poor communication/excluded": ["blindsided", "didn't know", "surprised", "no warning", "not told", "wasn't informed",
                                     "sprung", "nobody told", "didn't tell", "left out", "invisible", "nonexistent",
                                     "not great", "vague", "nobody really knew"],
    "training inadequate":         ["training was a joke", "wasn't enough", "too fast", "didn't cover", "hands-on",
                                     "quick walkthrough"],
    "old system was fine":         ["old system", "didn't need", "worked fine", "wasn't broken", "unnecessary",
                                     "wasn't asking for", "didn't ask"],
    "bell schedule chaos":         ["bell", "lunch", "schedule change", "moved around", "timing"],
    "reliability/glitches":        ["glitch", "cut out", "unreliable", "malfunction", "broke", "doesn't work",
                                     "garbled", "cuts out", "didn't ring"],
    "positive: improvement":       ["improvement", "upgrade", "better than", "clearer", "easier", "smooth",
                                     "net positive", "improved"],
    "positive: safety/emergency":  ["safety", "emergency", "lockdown", "fire", "evacuation", "security"],
    "support staff excluded":      ["support staff", "aide", "food service", "custodial", "we matter",
                                     "asked us", "staff too", "we're not"],
    "installation physical issues":["dust", "noise", "ceiling", "construction", "conduit", "installers"],
    "intercom interruptions":      ["interrupt", "middle of", "session", "counseling"],
}
