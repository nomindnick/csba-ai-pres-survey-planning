...
function UserLine({ text, delay }: { text: string; delay: number }) {
  return (
    <motion.div
      initial={{ opacity: 0 }}
      animate={{ opacity: 1 }}
    ...
    >
      <span className="shrink-0 select-none font-bold text-accent-phase2">❯</span>
      <motion.span
...
function AiResponse({ lines, delay }: { lines: string[]; delay: number }) {
  return (
    <div className="ml-4 mt-1 mb-3 border-l-2 border-accent-phase2/20 pl-4">
      {lines.map((line, i) => (
        <motion.div
          key={i}
...
export function SimulatedTerminal({ messages, title }: SimulatedTerminalProps) {
    ...
    <div className="rounded-xl border border-border-subtle bg-[#0c0e14] overflow-hidden shadow-2xl">
      ...
      <div className="p-5 font-mono text-[13px] leading-[1.7] max-h-[calc(100vh-280px)] overflow-y-auto">
        {messages.map((msg, i) => {
          ...
          if (msg.type === "user") {
            cumulativeDelay += 0.5;
            return <UserLine key={i} text={msg.content as string} delay={delay} />;
          }
          ...
          return <AiResponse key={i} lines={lines} delay={delay} />;
        })}
      </div>
    </div>
  );
